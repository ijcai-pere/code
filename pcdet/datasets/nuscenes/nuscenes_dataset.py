...
class NuScenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, ps_label_dir=None):
        ...
        super().__init__(dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger, ps_label_dir=ps_label_dir)
        self.infos = []
    ...
    def get_lidar_with_sweeps(self, index, max_sweeps=1):
        ...
        #     print('exist')
        # except:
        #     print(lidar_path, 'does not exist.')
        #     info = self.infos[0]
        #     lidar_path = self.root_path / info['lidar_path']
        #     points = np.fromfile(str(lidar_path), dtype=np.float32, count=-1).reshape([-1, 5])[:, :4]
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_cfg.get('USE_PSEUDO_LABEL', None) and self.training:
            pkt = self.load_last_pseudo_label()
            self.fill_pseudo_labels(input_dict, pkt)
