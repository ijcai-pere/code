...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None, ps_label_dir=None):
        ...
        self.data_augmentor = DataAugmentor(
            self.root_path, self.dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger, load_dir=self.ps_label_dir
    ...
    def load_last_pseudo_label(self):
        pkt, _ = self_training_utils.load_last_pseudo_label(self.ps_label_dir)  # ps has existed.
        return pkt

    def fill_pseudo_labels(self, input_dict, pkt):

        if input_dict['frame_id'] in pkt:
            gt_boxes = pkt[input_dict['frame_id']]['gt_boxes']
        else:
            raise ValueError('Cannot find pseudo label for frame: %s' % input_dict['frame_id'])
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            # print(data_dict['points'].shape, data_dict['gt_names'].shape, data_dict['gt_boxes'].shape)
            # print(len(num_points_in_gt),num_points_in_gt)
    ...
    def collate_batch(self, batch_list, _unused=False):
        ...
        ret['dataset_name'] = self.dataset_cfg['DATASET']
        return ret
