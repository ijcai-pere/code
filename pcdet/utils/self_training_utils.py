...
def load_last_pseudo_label(ps_label_dir):
    ps_label_list = glob.glob(os.path.join(ps_label_dir, 'ps_label_e*.pkl'))
    ps_label_list.sort(key=os.path.getmtime, reverse=True)
    last_pkl = ps_label_list[0]
    latest_pkl = pkl.load(open(last_pkl, 'rb'))
    return latest_pkl, last_pkl
...
def save_pseudo_label_epoch(model, val_loader, rank, leave_pbar, ps_label_dir, cur_epoch):
    ...
    val_dataloader_iter = iter(val_loader)#调用了fill
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        #print(pos_ps_batch_nmeters,ign_ps_batch_nmeters)
