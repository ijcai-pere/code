...
class Detector3DTemplate(nn.Module):
    ...
    # def use_src(self):
    #     self.model_cfg.ROI_HEAD.TRIPLET_LOSS.DATA_TYPE = 'src'
    #
    # def use_tar(self):
    #     self.model_cfg.ROI_HEAD.TRIPLET_LOSS.DATA_TYPE = 'tar'
    ...
    def post_processing_multicriterion(self, batch_dict):
        ...
        for index in range(batch_size):
            if batch_dict.get('batch_index', None) is not None:
                ...
                batch_mask = index
            len_list = batch_dict['len_list']
            len_preds = batch_dict['batch_cls_preds'][batch_mask].shape[0]
            box_preds = batch_dict['batch_box_preds'][batch_mask][:len_preds-len_list[index], ...]
            iou_preds = batch_dict['batch_cls_preds'][batch_mask][:len_preds-len_list[index], ...]
            cls_preds = batch_dict['roi_scores'][batch_mask][:len_preds-len_list[index], ...]
