...
Take Source Only model with pvrcnn on NuScenes -> KITTI  as an example:
```shell script
sh scripts/dist_train.sh ${NUM_GPUS} --cfg_file cfgs/da-nuscenes-kitti_models/pvrcnn/pvrcnn_pretrain.yaml \
...
```shell script
sh scripts/dist_train.sh ${NUM_GPUS} --cfg_file cfgs/da-nuscenes-kitti_models/pvrcnn_pere/pvrcnn_pere.yaml \
